    ...
    onBlock(instances, reason, data = {}) {
        ...
        switch (reason) {
            ...
            case 'userPermissions':
                return replyAll({ message, interaction }, embed('You are missing the following permissions:', missing.map(perm => `\`${util_1.default.permissions[perm]}\``).join(', ')));
            case 'modPermissions':
            ...
            case 'clientPermissions':
                return replyAll({ message, interaction }, embed('The bot is missing the following permissions:', missing.map(perm => `\`${util_1.default.permissions[perm]}\``).join(', ')));
            case 'throttling':
...
async function replyAll({ message, interaction }, options) {
    ...
    if (message) {
        return await message.reply({ ...options, ...util_1.default.noReplyPingInDMs(message) }).catch(() => null);
    }
    return null;
}
