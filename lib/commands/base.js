...
class Command {
    ...
    hasPermission(instances, ownerOverride = true) {
        ...
        if (!channel.isDMBased()) {
            if (modPermissions && !isModerator(member)) {
    ...
    onBlock(instances, reason, data = {}) {
        ...
        switch (reason) {
            ...
            case 'userPermissions': {
                ...
                return replyInteraction(instances, embed('You are missing the following permissions:', missing.map(perm => `\`${util_1.default.permissions[perm]}\``).join(', ')));
            }
            case 'modPermissions':
                return replyInteraction(instances, embed(useCommandOnlyIf('by "moderators"'), 'For more information visit the `page 3` of the `help` command.'));
            case 'clientPermissions': {
                if (!missing) {
                    throw new Error('Missing permissions object must be specified for "clientPermissions" case');
                }
                return replyInteraction(instances, embed('The bot is missing the following permissions:', missing.map(perm => `\`${util_1.default.permissions[perm]}\``).join(', ')));
            }
            case 'throttling': {
                if (!remaining) {
                    throw new Error('Remaining time value must be specified for "throttling" case');
                }
                return replyInteraction(instances, embed(`Please wait **${remaining.toFixed(1)} seconds** before using the \`${name}\` command again.`));
            }
        }
    }
    /**
...
async function replyInteraction(instances, options) {
    ...
    if ('interaction' in instances) {
        ...
        const payload = new discord_js_1.MessagePayload(interaction, options);
    ...
    if ('message' in instances) {
        ...
        const payload = new discord_js_1.MessagePayload(message, options);
