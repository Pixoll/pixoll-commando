...
class Util extends null {
    ...
    static noReplyPingInDMs(msg) {
        const options = msg.channel.type === discord_js_1.ChannelType.DM ? {
        ...
        if (Array.isArray(char)) {
            while (char.length > 0 && splitText.some(elem => elem.length > maxLength)) {
                ...
                if (currentChar instanceof RegExp) {
                    splitText = Util.removeNullishItems(splitText.flatMap(chunk => chunk.match(currentChar)));
                }
                else {
     ...
     */
    static removeNullishItems(array) {
        return array.filter((item) => typeof item !== 'undefined' && item !== null);
    }
    /**
