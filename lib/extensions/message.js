...
class CommandoMessage extends discord_js_1.Message {
    ...
    async run() {
        ...
        if (channel.type !== discord_js_1.ChannelType.DM && !guild.members.cache.has(client.user.id)) {
        ...
        if (channel.type !== discord_js_1.ChannelType.DM && hasPermission !== true) {
        ...
        if (channel.type !== discord_js_1.ChannelType.DM && command.clientPermissions) {
        ...
        try {
            ...
            if (util_1.default.probability(2)) {
                ...
                const vote = new discord_js_1.ButtonBuilder()
                    .setEmoji('👍')
                    .setLabel('Vote me')
                    .setStyle(discord_js_1.ButtonStyle.Link)
                    .setURL('https://top.gg/bot/802267523058761759/vote');
                const invite = new discord_js_1.ButtonBuilder()
                    .setEmoji('🔗')
                    .setLabel('Invite me')
                    .setStyle(discord_js_1.ButtonStyle.Link)
                    .setURL(botInvite);
                const row = new discord_js_1.ActionRowBuilder().addComponents(vote, invite);
    ...
    respond(options = {}) {
        ...
        if (type === 'reply' && channel.type === discord_js_1.ChannelType.DM)
            type = 'plain';
        if (type !== 'direct') {
            if (guild && channel.type !== discord_js_1.ChannelType.DM && !channel.permissionsFor(client.user)?.has('SendMessages')) {
                type = 'direct';
            }
        }
        if (content)
