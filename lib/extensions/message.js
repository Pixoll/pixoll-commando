...
class CommandoMessage extends discord_js_1.Message {
        ...
        if (!args && command.argsCollector) {
            ...
            if (collResult.cancelled) {
                if (collResult.prompts.length === 0 || collResult.cancelled === 'promptLimit') {
                    ...
                    return this.reply({ content: err.message, ...(0, util_1.noReplyInDMs)(this) });
                }
                client.emit('commandCancel', command, collResult.cancelled, this, collResult);
                // @ts-expect-error: this is not assignable to Message<boolean>
                return this.reply({ content: 'Cancelled command.', ...(0, util_1.noReplyInDMs)(this) });
            }
    ...
    respond({ type = 'reply', content = '', options = {}, lang = '', fromEdit = false } = {}) {
        ...
        if (content)
            options.content = resolveString(content);
        // @ts-expect-error: this is not assignable to Message<boolean>
        Object.assign(options, (0, util_1.noReplyInDMs)(this));
        switch (type) {
            case 'plain':
                if (!shouldEdit)
    ...
    editResponse(response, { type, options } = {}) {
        if (!response)
            return this.respond({ type, options, fromEdit: true });
