...
class CommandoClient extends discord_js_1.Client {
    ...
    get owners() {
        ...
        if (typeof owner === 'string')
            return util_1.default.filterNullishItems([users.resolve(owner)]);
        const owners = [];
        for (const user of owner)
            owners.push(users.resolve(user));
        return util_1.default.filterNullishItems(owners);
    }
    /**
    ...
    initDefaultListeners() {
        ...
        this.on('interactionCreate', interaction => {
            if (interaction.type !== discord_js_1.InteractionType.ApplicationCommand
                || interaction.channel?.type === discord_js_1.ChannelType.GuildStageVoice)
                return;
