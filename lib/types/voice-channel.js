...
function channelFilterExact(search) {
    return (chan) => chan.type === discord_js_1.ChannelType.GuildVoice && chan.name.toLowerCase() === search;
}
function channelFilterInexact(search) {
    return (chan) => chan.type === discord_js_1.ChannelType.GuildVoice && chan.name.toLowerCase().includes(search);
}
