...
function channelFilterExact(search: string) {
    return (chan: GuildBasedChannel) => chan.type === 'GUILD_VOICE' && chan.name.toLowerCase() === search;
}

function channelFilterInexact(search: string) {
    return (chan: GuildBasedChannel) => chan.type === 'GUILD_VOICE' && chan.name.toLowerCase().includes(search);
}
